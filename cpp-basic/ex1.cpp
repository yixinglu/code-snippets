...
// (x+y)/2
inline int avg(int x, int y) { return (x & y) + ((x ^ y) >> 1); }
...
void t4() {
  ...
  *(ptr++) += 123;
  printf(
      "%d, %d\n", *ptr,
      *(++ptr));  // printf parameters from right to left are pushed to stack.
}
...
void t5() {
  float a = 1.0f;
  cout << (int)a << " " << &a << " " << (int&)a << " " << boolalpha
       << ((int)a == (int&)a) << endl;
  float b = 0.0f;
  cout << (int)b << " " << &b << " " << (int&)b << " " << boolalpha
       << ((int)b == (int&)b) << endl;
}
